...
function renderBlogPage() {
  ...
  function renderArticles(filter) {
    ...
    return filtered.map(
      (article) => `
      <article data-id="${escapeHtml$6(article.id)}" style="border:1px solid #e8eaed;border-radius:16px;overflow:hidden;background:#fff;transition:box-shadow 0.15s;cursor:pointer" class="blog-card">
        <div style="background:#f8f9fa;padding:24px;font-size:40px;text-align:center">${article.emoji}</div>
        <div style="padding:20px">
          <div style="display:flex;gap:8px;align-items:center;margin-bottom:10px">
            <span style="font-size:11px;font-weight:600;color:#1a7a3c;background:#e8f5e9;padding:3px 10px;border-radius:12px">${escapeHtml$6(article.category)}</span>
            <span style="font-size:11px;color:#9aa0a6">${escapeHtml$6(article.readTime)}</span>
          </div>
          <h2 style="font-size:15px;font-weight:700;color:#202124;line-height:1.4;margin-bottom:8px">${escapeHtml$6(article.title)}</h2>
          <p style="font-size:13px;color:#5f6368;line-height:1.5;margin-bottom:14px">${escapeHtml$6(article.summary)}</p>
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:12px;color:#9aa0a6">${escapeHtml$6(article.date)} &bull; ${escapeHtml$6(article.author)}</span>
          </div>
        </div>
      </article>
    `
    ...
    <div style="min-height:100vh;background:#fff;display:flex;flex-direction:column">
      <div style="max-width:900px;margin:0 auto;padding:48px 24px;flex:1">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:32px" id="blog-cat-filters">
          ${categories.map(
    (c2, i) => `<button class="blog-cat-btn" data-cat="${escapeHtml$6(c2)}" style="padding:6px 16px;border-radius:20px;border:1px solid ${i === 0 ? "#1a7a3c" : "#dadce0"};background:${i === 0 ? "#1a7a3c" : "#fff"};color:${i === 0 ? "#fff" : "#3c4043"};font-size:13px;cursor:pointer;font-weight:500">${escapeHtml$6(c2)}</button>`
  ).join("")}
        </div>
...
async function renderAdminPage() {
  ...
  try {
    ...
    if (myPrincipal) {
      await backend2.assignCallerUserRole(myPrincipal, UserRole.admin);
      ...
      try {
        ...
        if (claimed) {
          ...
          if (claimStatus) {
            ...
            claimStatus.textContent = "Admin already exists. You cannot claim admin access.";
          }
          showToast(
    ...
    (l, i) => `
          <div data-ocid="admin.item.${i + 1}" class="p-5 flex items-start gap-4" style="border-bottom:1px solid oklch(var(--border))">
            <div class="flex-1 min-w-0">
              <h3 class="font-semibold" style="color:oklch(var(--foreground))">${escapeHtml$5(l.name)}</h3>
              <p class="text-sm mt-0.5" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(getCategoryName$2(l.categoryId))} &bull; ${escapeHtml$5(l.city)}, ${escapeHtml$5(l.state)}</p>
              <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(l.address)}</p>
              <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(l.description).substring(0, 120)}...</p>
            </div>
  ...
  content.innerHTML = `
    ...
    (cat, i) => `
              <tr data-ocid="admin.item.${i + 1}" class="border-b" style="border-color:oklch(var(--border))">
                ...
                <td class="py-3 px-4 text-sm font-medium" style="color:oklch(var(--foreground))">${escapeHtml$5(cat.name)}</td>
                <td class="py-3 px-4 text-sm" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(cat.description)}</td>
                <td class="py-3 px-4">
...
async function loadVendors() {
  ...
  content.innerHTML = `
    ...
    (v2, i) => `
              <tr data-ocid="admin.item.${i + 1}" class="border-b" style="border-color:oklch(var(--border))">
                <td class="py-3 px-4 text-sm font-medium" style="color:oklch(var(--foreground))">${escapeHtml$5(v2.name)}</td>
                <td class="py-3 px-4 text-sm" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(v2.businessName)}</td>
                <td class="py-3 px-4 text-sm" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(v2.phone)}</td>
                <td class="py-3 px-4 text-xs font-mono truncate max-w-xs" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(v2.principal.toString())}</td>
              </tr>
            `
  ).join("")}
          </tbody>
        ...
        <div data-ocid="admin.item.${i + 1}" class="p-5 flex items-start gap-4" style="border-bottom:1px solid oklch(var(--border))">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              <span class="text-xs font-semibold px-2 py-0.5 rounded-full" style="background:#e8f5e9;color:#1a7a3c">${escapeHtml$5(a2.category)}</span>
              <span class="text-xs" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(a2.readTime)}</span>
            </div>
            <h3 class="font-semibold text-sm" style="color:oklch(var(--foreground))">${escapeHtml$5(a2.title)}</h3>
            <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(a2.summary).substring(0, 100)}...</p>
            <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$5(a2.date)} &bull; ${escapeHtml$5(a2.author)}</p>
          </div>
          <button data-delete-article="${escapeHtml$5(a2.id)}" data-ocid="admin.delete_button" class="flex-shrink-0 px-3 py-1.5 text-xs font-semibold rounded-lg" style="color:oklch(var(--destructive));border:1px solid oklch(var(--destructive))">Delete</button>
        </div>
      `
  ).join("")}
    </div>
  ...
  (_g = document.getElementById("listing-form")) == null ? void 0 : _g.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      await backend2.submitListing({
        name: (_a4 = data.get("name")) == null ? void 0 : _a4.trim(),
        categoryId: BigInt(data.get("category")),
...
function loadCachedLocation() {
  ...
  if (lat && lng) {
    return {
      ...
      city: localStorage.getItem("dhoondho_city") || void 0,
      state: localStorage.getItem("dhoondho_state") || void 0
    };
  }
  return null;
}
...
function attachHomeEvents(_authed) {
  ...
  (_d = document.getElementById("home-mic-btn")) == null ? void 0 : _d.addEventListener("click", () => {
    ...
    r2.onerror = () => alert(t("couldNotCaptureVoice"));
  });
  (_e = document.getElementById("home-loc-btn")) == null ? void 0 : _e.addEventListener("click", doNearby);
  ...
  main.innerHTML = `
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav class="flex items-center gap-2 text-sm mb-6" style="color:oklch(var(--muted-foreground))">
        ...
        <span>/</span>
        <span>${escapeHtml$1(listing.name)}</span>
      </nav>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
          <div class="bg-white rounded-2xl border p-7 mb-6" style="border-color:oklch(var(--border))">
            <div class="flex items-start gap-4 mb-5">
              ...
              <div class="flex-1 min-w-0">
                <h1 class="text-2xl font-bold mb-1" style="color:oklch(var(--foreground))">${escapeHtml$1(listing.name)}</h1>
            ...
            <div class="space-y-3 mb-6">
              ...
              ${listing.phone ? `<div class="flex items-center gap-3">
                ...
                <a href="tel:${escapeHtml$1(listing.phone)}" class="text-sm font-medium no-underline hover:underline" style="color:oklch(var(--primary))">${escapeHtml$1(listing.phone)}</a>
              </div>` : ""}
              ${listing.website ? `<div class="flex items-center gap-3">
                ...
                <a href="${escapeHtml$1(listing.website)}" target="_blank" rel="noopener" class="text-sm font-medium no-underline hover:underline truncate" style="color:oklch(var(--primary))">${escapeHtml$1(listing.website)}</a>
              </div>` : ""}
            </div>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-white rounded-2xl border p-5" style="border-color:oklch(var(--border))">
            ...
            <div class="space-y-3">
              ${listing.phone ? `<a href="tel:${escapeHtml$1(listing.phone)}" data-ocid="listing.button" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-white no-underline text-center" style="background:oklch(var(--primary))">
    ...
    try {
      ...
      return ids.includes(String(listing.id)) ? "Saved" : "Save Listing";
    } catch {
      return "Save Listing";
    }
  })()}</span>
              ...
              ${listing.website ? `<a href="${escapeHtml$1(listing.website)}" target="_blank" rel="noopener" data-ocid="listing.link" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold border no-underline" style="border-color:oklch(var(--border));color:oklch(var(--foreground))">
...
function showModal(html) {
  ...
  overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.5);backdrop-filter:blur(2px);z-index:1000;display:flex;align-items:center;justify-content:center;padding:16px";
...
function attachListingEvents(listingId) {
  ...
  (_a3 = document.getElementById("contact-btn")) == null ? void 0 : _a3.addEventListener("click", () => {
    var _a4, _b3;
    ...
    `);
    (_a4 = overlay.querySelector("#close-modal-btn")) == null ? void 0 : _a4.addEventListener("click", () => overlay.remove());
    (_b3 = overlay.querySelector("#lead-form")) == null ? void 0 : _b3.addEventListener("submit", (e) => {
      e.preventDefault();
      const form = e.target;
      const data = new FormData(form);
      const lead = {
  ...
  (_b2 = document.getElementById("book-btn")) == null ? void 0 : _b2.addEventListener("click", () => {
    var _a4, _b3;
    const overlay = showModal(`
    ...
    `);
    (_a4 = overlay.querySelector("#close-book-btn")) == null ? void 0 : _a4.addEventListener("click", () => overlay.remove());
    (_b3 = overlay.querySelector("#book-form")) == null ? void 0 : _b3.addEventListener("submit", (e) => {
      e.preventDefault();
      const form = e.target;
      const data = new FormData(form);
      const booking = {
  ...
  (_c = document.getElementById("submit-review-btn")) == null ? void 0 : _c.addEventListener("click", () => {
    var _a4;
    if (selectedRating === 0) {
    ...
    container.innerHTML = `
      ...
      (r2, i) => `
        <div data-ocid="listing.item.${i + 1}" style="border-bottom:1px solid #f0f0f0;padding:12px 0">
          ...
          ${r2.text ? `<p style="font-size:13px;color:#3c4043;margin:0">${r2.text}</p>` : ""}
