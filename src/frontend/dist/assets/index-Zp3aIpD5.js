...
function renderBlogPage() {
  ...
  function renderArticles(filter) {
    ...
    return filtered.map(
      (article) => `
      <article data-id="${escapeHtml$5(article.id)}" style="border:1px solid #e8eaed;border-radius:16px;overflow:hidden;background:#fff;transition:box-shadow 0.15s;cursor:pointer" class="blog-card">
        <div style="background:#f8f9fa;padding:24px;font-size:40px;text-align:center">${article.emoji}</div>
        <div style="padding:20px">
          <div style="display:flex;gap:8px;align-items:center;margin-bottom:10px">
            <span style="font-size:11px;font-weight:600;color:#1a7a3c;background:#e8f5e9;padding:3px 10px;border-radius:12px">${escapeHtml$5(article.category)}</span>
            <span style="font-size:11px;color:#9aa0a6">${escapeHtml$5(article.readTime)}</span>
          </div>
          <h2 style="font-size:15px;font-weight:700;color:#202124;line-height:1.4;margin-bottom:8px">${escapeHtml$5(article.title)}</h2>
          <p style="font-size:13px;color:#5f6368;line-height:1.5;margin-bottom:14px">${escapeHtml$5(article.summary)}</p>
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:12px;color:#9aa0a6">${escapeHtml$5(article.date)} &bull; ${escapeHtml$5(article.author)}</span>
          </div>
        </div>
      </article>
    `
    ...
    <div style="min-height:100vh;background:#fff;display:flex;flex-direction:column">
      <div style="max-width:900px;margin:0 auto;padding:48px 24px;flex:1">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:32px" id="blog-cat-filters">
          ${categories.map(
    (c2, i) => `<button class="blog-cat-btn" data-cat="${escapeHtml$5(c2)}" style="padding:6px 16px;border-radius:20px;border:1px solid ${i === 0 ? "#1a7a3c" : "#dadce0"};background:${i === 0 ? "#1a7a3c" : "#fff"};color:${i === 0 ? "#fff" : "#3c4043"};font-size:13px;cursor:pointer;font-weight:500">${escapeHtml$5(c2)}</button>`
  ).join("")}
        </div>
    ...
    (l, i) => `
          <div data-ocid="admin.item.${i + 1}" class="p-5 flex items-start gap-4" style="border-bottom:1px solid oklch(var(--border))">
            <div class="flex-1 min-w-0">
              <h3 class="font-semibold" style="color:oklch(var(--foreground))">${escapeHtml$4(l.name)}</h3>
              <p class="text-sm mt-0.5" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(getCategoryName$2(l.categoryId))} &bull; ${escapeHtml$4(l.city)}, ${escapeHtml$4(l.state)}</p>
              <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(l.address)}</p>
              <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(l.description).substring(0, 120)}...</p>
            </div>
  ...
  content.innerHTML = `
    ...
    (cat, i) => `
              <tr data-ocid="admin.item.${i + 1}" class="border-b" style="border-color:oklch(var(--border))">
                ...
                <td class="py-3 px-4 text-sm font-medium" style="color:oklch(var(--foreground))">${escapeHtml$4(cat.name)}</td>
                <td class="py-3 px-4 text-sm" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(cat.description)}</td>
                <td class="py-3 px-4">
...
async function loadVendors() {
  ...
  content.innerHTML = `
    ...
    (v2, i) => `
              <tr data-ocid="admin.item.${i + 1}" class="border-b" style="border-color:oklch(var(--border))">
                <td class="py-3 px-4 text-sm font-medium" style="color:oklch(var(--foreground))">${escapeHtml$4(v2.name)}</td>
                <td class="py-3 px-4 text-sm" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(v2.businessName)}</td>
                <td class="py-3 px-4 text-sm" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(v2.phone)}</td>
                <td class="py-3 px-4 text-xs font-mono truncate max-w-xs" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(v2.principal.toString())}</td>
              </tr>
            `
  ).join("")}
          </tbody>
        ...
        <div data-ocid="admin.item.${i + 1}" class="p-5 flex items-start gap-4" style="border-bottom:1px solid oklch(var(--border))">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              <span class="text-xs font-semibold px-2 py-0.5 rounded-full" style="background:#e8f5e9;color:#1a7a3c">${escapeHtml$4(a2.category)}</span>
              <span class="text-xs" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(a2.readTime)}</span>
            </div>
            <h3 class="font-semibold text-sm" style="color:oklch(var(--foreground))">${escapeHtml$4(a2.title)}</h3>
            <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(a2.summary).substring(0, 100)}...</p>
            <p class="text-xs mt-1" style="color:oklch(var(--muted-foreground))">${escapeHtml$4(a2.date)} &bull; ${escapeHtml$4(a2.author)}</p>
          </div>
          <button data-delete-article="${escapeHtml$4(a2.id)}" data-ocid="admin.delete_button" class="flex-shrink-0 px-3 py-1.5 text-xs font-semibold rounded-lg" style="color:oklch(var(--destructive));border:1px solid oklch(var(--destructive))">Delete</button>
        </div>
      `
  ).join("")}
    </div>
  ...
  if (loc == null ? void 0 : loc.city) {
    ...
    row.innerHTML = `
      <p style="font-size:13px;color:#5f6368;margin:0;display:flex;align-items:center;gap:6px">
        ...
        <strong>${t("location_detected")}</strong>&nbsp;${escapeHtml$2(place)}
      </p>
    `;
  } else {
    row.innerHTML = `
...
async function renderHomePage() {
  ...
  const footerLocHTML = (cachedLoc == null ? void 0 : cachedLoc.city) ? `<p style="font-size:13px;color:#5f6368;margin:0;display:flex;align-items:center;gap:6px">
        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1a7a3c" stroke-width="2"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
        <strong>${t("location_detected")}</strong>&nbsp;${escapeHtml$2(cachedLoc.state ? `${cachedLoc.city}, ${cachedLoc.state}` : cachedLoc.city)}
       </p>` : `<p style="font-size:13px;color:#9aa0a6;margin:0;display:flex;align-items:center;gap:6px">
        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#9aa0a6" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        ${t("enable_location")}
       </p>`;
  main.innerHTML = `
